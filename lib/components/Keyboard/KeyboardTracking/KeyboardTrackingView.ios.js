...
class KeyboardTrackingView extends PureComponent {
  ...
  componentDidUpdate(prevProps) {
    const {bottomViewColor} = prevProps;

    if (this.props.bottomViewColor !== bottomViewColor) {
      if (this.ref && KeyboardTrackingViewTempManager && KeyboardTrackingViewTempManager.updateBottomView) {
        KeyboardTrackingViewTempManager.updateBottomView(
          ReactNative.findNodeHandle(this.ref),
          this.props.bottomViewColor
        );
      }
    }
  }
