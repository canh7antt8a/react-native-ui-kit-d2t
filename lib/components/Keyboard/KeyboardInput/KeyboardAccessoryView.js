...
class KeyboardAccessoryView extends Component {
  static propTypes = {
    ...
    /**
     * iOS only.
     * Changes the default SafeArea color from `addBottomView` prop
     *
     */
  ...
  componentWillMount() {
    ...
    this.keyboardDidShowListener = Keyboard.addListener('keyboardWillShow', (e) => {
      self.setState({
        keyBoardOffsetHeight: e.endCoordinates.height
      }, () => self.forceUpdate())
    });
  ...
  checkDeviceIphoneNotBunnyEar = () =>{
    let model = DeviceInfo.getModel()
    if(model.includes('iPhone 7')){
      return true
    }if(model.includes('iPhone 8')){
      return true
  ...
  render() {
    ...
    return <View style={{
      ...
      paddingBottom: this.state.keyBoardOffsetHeight == 0 ? (Platform.OS == "ios" && !this.checkDeviceIphoneNotBunnyEar()) ? 20 : 0 : 0, 
      bottom: this.state.keyBoardOffsetHeight, width: '100%'
    }}>
