...
const Chip = ({
  ...
  const renderBadge = useCallback(() => {
    return <Badge size={BADGE_SIZES.default} testID={`${testID}.counter`} {...badgeProps} // @ts-ignore
  ...
  const renderOnDismiss = useCallback(() => {
    return <TouchableOpacity style={[getMargins('dismiss'), dismissContainerStyle]} onPress={onDismiss} hitSlop={{
      ...
      right: 10
        ...
        <Image // @ts-ignore
      source={dismissIcon} tintColor={dismissColor} style={[dismissIconStyle]} accessibilityLabel="dismiss" testID={`${testID}.dismiss`} />
      </TouchableOpacity>;
  ...
  const renderLabel = useCallback(() => {
    return <Text text90M numberOfLines={1} // @ts-ignore
    style={[styles.label, getMargins('label'), labelStyle]} testID={`${testID}.label`}>
        {!!label && label}
      </Text>;
  }, [label, labelStyle]);
