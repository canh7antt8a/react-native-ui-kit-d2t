...
class FloatingButton extends PureComponent {
  ...
  render() {
    ...
    const Container = !withoutAnimation ? AnimatableView : View; // NOTE: keep this.firstLoad as true as long as the visibility changed to true
    ...
    return <Container pointerEvents="box-none" style={[styles.animatedContainer, Constants.isAndroid && {
      zIndex: 99
    }]} animation={!visible ? 'fadeOutDown' : 'fadeInUp'} duration={SHOW_ANIMATION_DURATION} delay={!visible ? HIDE_ANIMATION_DURATION : SHOW_ANIMATION_DELAY} easing={'ease-out'} onAnimationEnd={this.onAnimationEnd}>
        {this.renderOverlay()}
        {this.renderButton()}
